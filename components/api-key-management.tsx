...
import { Key, Save, Check, AlertCircle, ExternalLink, RefreshCw } from "lucide-react"
...
export default function ApiKeyManagement({ onStatusChange }: ApiKeyManagementProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.geminiAvailable) {
          setApiStatus({
            status: "valid",
            message: `API key is valid and working with model: ${data.model || "Gemini"}`,
            model: data.model,
          })
          setIsConfigured(true)
          if (onStatusChange) {
            onStatusChange({ isConfigured: true, model: data.model })
          }
        } else {
          setApiStatus({
            status: "invalid",
          ...
          if (onStatusChange) {
            onStatusChange({ isConfigured: false })
          }
        }
      } else {
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      if (data.valid) {
        // Store the API key in localStorage for demonstration purposes
        ...
        toast({
          title: "API Key Saved",
          description: `Your Gemini API key has been validated and saved successfully. Using model: ${data.model || "Gemini"}`,
        })
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6 shadow-md">
      <div className="mb-4 flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={checkApiStatus} disabled={isChecking} className="gap-1">
      ...
      <div className="mb-6">
        <p className="mb-4 text-sm text-muted-foreground">
          Enter your Gemini API key to enable AI image generation. You can get a key from the{" "}
          <a
      ...
      <div className="rounded-md bg-secondary/50 p-3 text-xs text-muted-foreground">
        ...
        <ul className="mt-1 list-disc pl-4 space-y-1">
          ...
          <li>For this demo, the key is stored in your browser's local storage</li>
          <li>In production, API keys should be stored securely on the server</li>
          <li>
            <Link href="/settings" className="text-primary hover:underline">
              Go to Settings
            </Link>{" "}
            to configure your API key permanently
          </li>
        </ul>
      </div>
    </div>
